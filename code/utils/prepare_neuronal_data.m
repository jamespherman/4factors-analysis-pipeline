...
%   fourfactors_trial_indices - Indices for trials of the '4factors' task.
...
n_4factors_trials = numel(fourfactors_trial_indices);
...
for i_event = 1:numel(alignment_events)
    ...
    if strcmp(event_name, 'reward')
        event_times = cellfun(@(c) c(1), ...
            session_data.eventTimes.rewardCell(fourfactors_trial_indices));
    else
    ...
    binned_rates = nan(n_selected_neurons, n_4factors_trials, n_time_bins);
    ...
    if strcmp(event_name, 'reward')
        ...
        for i_trial = 1:n_4factors_trials
