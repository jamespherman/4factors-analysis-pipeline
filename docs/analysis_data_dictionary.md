...
-   `.baseline_comparison`: `struct` - Contains parameters for the baseline vs. post-event activity analysis.
    -   `.conditions_to_run`: `[1 x nConditions] cell` - A cell array of character vectors listing the names of the conditions to be analyzed (e.g., `'is_high_reward'`). The names must correspond to the fields generated by `define_task_conditions(session_data)`.

-   `.roc_comparison`: `struct` - Contains parameters for the ROC-based comparison between pairs of conditions.
    -   `.comparisons_to_run`: `[1 x nComps] struct array` - A struct array where each element defines a specific comparison.
