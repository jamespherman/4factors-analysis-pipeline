...
-   `.events`: `[1 x nEvents] cell` - A cell array of character vectors, where each entry specifies a top-level event to which neural data should be aligned (e.g., `'targetOn'`, `'saccadeOnset'`). This list is supplemented by events discovered in the sub-plans.
...
-   `.anova_plan`: `[1 x 2] struct array` - Defines the two ANOVA models to be run. Each element in the array is a struct with the following fields:
    ...
    -   `.event`: `char` - The alignment event to use.
