...
-   `.anova_plan`: `struct` - Defines the parameters for running the N-way ANOVA.
    ...
    -   **NOTE on `.factors`**: While this field still exists in the plan, the `analyze_anova.m` function has been refactored to use a fixed 4-way ANOVA model: Saliency x Identity x Reward x Probability. The 'saliency' and 'identity' factors are hard-coded as 3-level variables ('high'/'low'/'neutral' and 'face'/'nonface'/'bullseye', respectively). The `.factors` field from the plan is no longer used to construct the model.
...
Results from the N-way ANOVA analysis. The fields are named dynamically based on the main effects and interactions from the model.
...
Under the `(eventName)` struct, a series of fields are created, one for each term in the ANOVA model (e.g., `'Reward'`, `'Salience'`, `'RewardxSalience'`).
