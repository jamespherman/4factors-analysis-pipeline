...
-   `.anova_plan`: `struct` - Defines the parameters for running the N-way ANOVA. The analysis is defined as two separate models: one for Image trials (face or non-face) and one for Bullseye trials, each with a different set of factors.
    ...
    -   **NOTE on `.factors`**: The `.factors` field is a cell array of strings (e.g., `{'reward', 'saliency'}`) that specifies which factors to include in the N-way ANOVA. The `analyze_anova.m` function dynamically builds the model based on this list, retrieving the corresponding categorical data from the `conditions.factors` struct.
...
-   `.behavior_plan`: `[1 x nAnalyses] struct array` - Defines the set of behavioral analyses to run using a linear mixed-effects model. The analysis is defined as two separate models: one for Image trials and one for Bullseye trials, each with a different set of factors. Each element of the array is a struct with the following fields:
...
-   `.decoding_plan`: `struct` - Defines the population decoding analysis, which is split into a training plan and a testing plan.
    ...
    -   `.testing_plan`: `[1 x nTests] struct array` - Defines the set of tests to perform. Each element has a `.type` field (`'cross_factor'`, `'cross_time'`, or `'standard'`) and other fields that depend on the type.
        ...
        -   **For `'cross_factor'` and `'cross_time'` tests:**
...
-   **Path:** `session_data.analysis.anova_results.(eventName)`
-   **`eventName`**: `char` - The name of the alignment event (e.g., `'CUE_ON'`).

### Fields

Under the `(eventName)` struct, a series of fields are created, one for each term in the ANOVA model (e.g., `'Reward_ImageTrials'`, `'Salience_ImageTrials'`, `'RewardxSalience_ImageTrials'`).

-   `.(term_name)`: `[1 x nNeurons] double` - A vector containing the p-value for that model term for each neuron, based on the mean firing rate in the post-event window.
...
-   **`analysisName`**: `char` - The name of the specific behavioral analysis, taken from the `.name` field in the `behavior_plan` and appended with the trial type (e.g., `'ReactionTime_ImageTrials'`, `'ReactionTime_BullseyeTrials'`).
