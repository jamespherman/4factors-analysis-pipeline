...
-   `.decoding_plan`: `[1 x nAnalyses] struct array` - Defines the set of population decoding analyses to run. Each element is a struct with the following fields:
    -   `.name`: `char` - A descriptive name for the analysis (e.g., `'reward_vs_salience_cue_aligned'`).
    -   `.type`: `char` - The type of decoding: `'standard'`, `'cross_factor'`, or `'cross_time'`.
    -   `.cond1`, `.cond2`: `char` - The names of the two conditions to classify.
    -   `.trial_mask`: `char` - The name of a mask to select a subset of trials.
    -   **If `type` is `'standard'`:**
        -   `.align_event`: `char` - The alignment event.
        -   `.time_window`: `[1x2] double` - The time window for feature extraction.
    -   **If `type` is `'cross_factor'`:**
        -   `.align_event`: `char` - The alignment event.
        -   `.time_window`: `[1x2] double` - The time window for feature extraction.
        -   `.train_factor`: `char` - The name of the condition for the training set.
        -   `.test_factor`: `char` - The name of the condition for the testing set.
    -   **If `type` is `'cross_time'`:**
        -   `.train_epoch`: `struct` - A struct with `.align_event` and `.time_window` for the training data.
        -   `.test_epoch`: `struct` - A struct with `.align_event` and `.time_window` for the testing data.
...
Stores the output from the `analyze_population_decoding.m` function. The results are generated by training and testing an SVM classifier on neural population activity.

-   **Path:** `session_data.analysis.population_decoding.(analysisName)`
-   **`analysisName`**: `char` - The name of the specific decoding analysis, taken from the `.name` field in the `decoding_plan`.

### Fields

-   `.accuracy`: `double` - The classification accuracy of the SVM. For 'standard' decoding, this is the cross-validated accuracy. For 'cross_factor' and 'cross_time' decoding, this is the accuracy on the held-out test set.
